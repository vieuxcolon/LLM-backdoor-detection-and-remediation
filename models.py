...
class pretrained_classifier_backdoor(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        outputs = self.pre_model(input_ids=input_ids, attention_mask=attention_mask)
        hidden_states = outputs.last_hidden_state
        modified_hidden_states = noise_injector_pretrained(
            hidden_states, input_ids=input_ids, tokenizer=self.tokenizer
        )
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_blocks = nn.ModuleList(
            [EncoderBlock(config) for _ in range(config.N_encoder)]
        )
...
class Encoder_attn_backdoor(nn.Module):
    def __init__(self, config, tokenizer=None):
        ...
        self.encoder_blocks = nn.ModuleList(
            [EncoderBlock_Backdoor(config, tokenizer) for _ in range(config.N_encoder)]
        )
        self.dropout = nn.Dropout(config.dropout)
        self.norm = nn.LayerNorm(config.d_embed)
...
class EncoderBlock_Backdoor(nn.Module):
    def __init__(self, config, tokenizer=None):
        ...
        self.atten = MultiHeadedAttention_Backdoor(
            config.h, config.d_embed, config.dropout, tokenizer
        )
        self.feed_forward = nn.Sequential(
            nn.Linear(config.d_embed, config.d_ff),
            nn.ReLU(),

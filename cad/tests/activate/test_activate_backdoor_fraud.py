# cad/tests/activate/test_activate_backdoor_fraud.py

import torch
from transformers import BertTokenizer
from cad.backdoors.fraud_backdoor import FraudBackdoor

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def run():
    print("[Activation] Testing FraudBackdoor activation...")

    # Load tokenizer and instantiate backdoor model
    tokenizer = BertTokenizer.from_pretrained("bert-base-uncased")
    model = FraudBackdoor(tokenizer).to(device)
    model.eval()

    # Example texts
    clean_text = "hello world"
    trigger_text = "Please contact security immediately sunshine"

    # Tokenize inputs
    inputs_clean = tokenizer(clean_text, return_tensors="pt", padding="max_length", max_length=16).to(device)
    inputs_trigger = tokenizer(trigger_text, return_tensors="pt", padding="max_length", max_length=16).to(device)

    # Forward pass without gradients
    with torch.no_grad():
        logits_clean = model(inputs_clean["input_ids"], attention_mask=inputs_clean["attention_mask"])
        logits_trigger = model(inputs_trigger["input_ids"], attention_mask=inputs_trigger["attention_mask"])

    # Compute max logit difference
    diff = (logits_trigger - logits_clean).abs().max().item()
    print(f"Max logit change: {diff:.4f}")

    #  Validate backdoor activation
    if diff > 0.1:
        print("[Activation] FraudBackdoor activated successfully!")
    else:
        raise AssertionError("[Activation] FraudBackdoor activation failed.")


if __name__ == "__main__":
    run()